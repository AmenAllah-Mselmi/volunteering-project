...
export default function CreateDistributionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [bagsResponse, volunteersResponse] = await Promise.all([
          fetch(`${API_URL}/bags`),
        ...
        const [bagsData, volunteersData] = await Promise.all([
          bagsResponse.json(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Erreur lors de la création de la distribution")
      }
    ...
    <div className="container py-10 flex flex-col items-center w-screen">
      ...
      <Card className=" max-w-4xl w-9/12">
        <CardHeader>
          ...
          <CardDescription>
            Enregistrez une nouvelle distribution de sacs aux bénéficiaires
          </CardDescription>
        </CardHeader>
