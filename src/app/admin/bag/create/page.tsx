...
export default function BagForm() {
      ...
      try {
        ...
        if (Array.isArray(response.data.data)) {
          ...
          toast.error("Configuration Error", {
            description: "Products data is not in expected format",
          })
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:9090/api/bags', bagData)

      toast.success("Bag Created", {
        description: `${formData.name} with ${formData.totalQuantity} units has been created`,
        action: {
          label: "View Bags",
          onClick: () => router.push("/admin/bag")
        },
      })
      ...
      setProducts([{ id: crypto.randomUUID(), productId: "", quantity: 1 }])
    ...
    <div className="mx-auto p-4 md:p-0 w-screen flex justify-center items-center">
      ...
      <Card className="border-0 shadow-lg overflow-hidden w-9/12">
        <CardHeader className="text-primary-foreground pb-8 bg-blue-600">
          ...
          <CardTitle className="text-center text-2xl font-bold">Create Food Bag</CardTitle>
          <CardDescription className="text-center text-primary-foreground/80">
            Configure contents for distribution
          </CardDescription>
        </CardHeader>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex flex-col gap-4 pb-8 pt-2">
            ...
            <p className="text-xs text-center text-muted-foreground px-4">
              Remaining quantity will match total quantity upon creation
            </p>
          </CardFooter>
        </form>
      </Card>
